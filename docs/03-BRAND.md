# BRAND.md

> Documento canônico da identidade da marca Fyrmma.
>
> Define como queremos ser percebidos, como nos comunicamos e quais sensações nossa marca deve transmitir.
>
> Toda decisão relacionada à comunicação, design, experiência do usuário e conteúdo deve estar alinhada com este documento.

---

# Essência da Marca

A Fyrmma existe para transformar operações manuais em processos digitais inteligentes.

Não desenvolvemos tecnologia pela tecnologia.

Projetamos soluções que ajudam empresas de serviços a crescer com mais organização, eficiência e previsibilidade.

Nosso papel é traduzir problemas complexos em soluções simples.

---

# Posicionamento

A Fyrmma ocupa um espaço entre:

- consultoria;
- engenharia de software;
- automação de processos;
- inteligência artificial aplicada.

Não somos uma agência.

Não somos apenas uma software house.

Também não somos uma consultoria tradicional.

Somos uma empresa de engenharia digital.

---

# Personalidade da Marca

Se a Fyrmma fosse uma pessoa, seria:

- técnica sem ser arrogante;
- experiente sem parecer distante;
- organizada;
- confiável;
- objetiva;
- curiosa;
- analítica;
- transparente;
- pragmática.

Nunca buscamos parecer extravagantes.

Preferimos transmitir competência.

---

# Como queremos ser percebidos

Queremos que nossos clientes pensem:

"Essa empresa realmente entende como meu negócio funciona."

"Não estão tentando me vender tecnologia."

"Eles parecem organizados."

"Transmitem confiança."

"Sabem exatamente o que estão fazendo."

---

# O que NÃO queremos transmitir

Nunca queremos parecer:

- uma agência de marketing;
- uma empresa de design;
- vendedores de IA;
- vendedores de sites;
- uma software house genérica;
- excessivamente corporativos;
- burocráticos;
- exageradamente futuristas.

---

# Voz da Marca

Nossa comunicação deve ser:

- clara;
- humana;
- objetiva;
- técnica quando necessário;
- acessível para pessoas não técnicas.

Evitamos linguagem excessivamente comercial.

---

# Linguagem

Preferimos palavras como:

- eficiência
- processos
- operação
- organização
- integração
- automação
- engenharia
- evolução
- crescimento
- simplicidade

Evitamos:

- revolução
- disruptivo
- potencialize
- inovação sem contexto
- solução completa
- líder de mercado
- transformação digital (como clichê)
- empresa 4.0

---

# Tom de Voz

## Conversamos como especialistas.

Não como vendedores.

Explicamos.

Orientamos.

Educamos.

Depois vendemos.

---

## Evitamos exageros

Não utilizamos frases como:

"O melhor."

"O mais moderno."

"A solução definitiva."

"Resultados garantidos."

A credibilidade vem da consistência.

---

## Escrevemos para pessoas

Mesmo quando tratamos de assuntos técnicos.

Sempre priorizamos clareza.

---

# Identidade Visual

Nossa identidade deve transmitir:

- organização;
- precisão;
- engenharia;
- tecnologia;
- confiança.

Não buscamos uma estética chamativa.

Buscamos uma estética sólida.

---

# Direção Visual

A interface deve ser:

- limpa;
- moderna;
- consistente;
- sofisticada;
- discreta.

O design deve valorizar:

- espaçamento;
- tipografia;
- contraste;
- hierarquia visual.

Evitamos excesso de:

- gradientes;
- brilhos;
- animações;
- efeitos visuais.

---

# Inteligência Artificial

A IA faz parte do nosso processo.

Ela não deve dominar nossa comunicação.

Não queremos ser percebidos como:

"mais uma empresa de IA."

Queremos ser percebidos como uma empresa que utiliza IA com responsabilidade para entregar soluções melhores.

---

# Conteúdo

Todo conteúdo publicado pela Fyrmma deve cumprir pelo menos um destes objetivos:

- ensinar;
- esclarecer;
- reduzir objeções;
- demonstrar experiência;
- compartilhar conhecimento.

Nunca publicamos conteúdo apenas para gerar volume.

---

# Experiência do Cliente

Cada interação com a marca deve transmitir:

- organização;
- rapidez;
- transparência;
- clareza;
- profissionalismo.

Do primeiro acesso ao site até a entrega do projeto.

---

# Resumo

A Fyrmma deve ser percebida como uma empresa de engenharia digital que ajuda empresas de serviços a organizar, automatizar e evoluir suas operações através de software, automação e inteligência artificial aplicada.

Nossa credibilidade vem da clareza, da simplicidade e da consistência.

Nunca do exagero.

---

# Escopo deste Documento

Este documento define a identidade da marca.

Os demais documentos possuem responsabilidades específicas:

- BUSINESS.md → modelo de negócio.
- PRINCIPLES.md → princípios da empresa.
- WEBSITE.md → arquitetura do site.
- COPY.md → comunicação institucional.
- ROADMAP.md → prioridades do projeto.
- AGENTS.md → instruções para agentes de IA.
