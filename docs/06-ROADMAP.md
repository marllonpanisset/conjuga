# ROADMAP.md

> Documento canônico da evolução estratégica da Fyrmma.
>
> Define as principais fases de crescimento da empresa e suas prioridades.
>
> Este documento não substitui um backlog de desenvolvimento. Seu objetivo é orientar decisões de longo prazo.

---

# Visão

A Fyrmma será construída de forma incremental.

Cada etapa deve gerar valor antes da próxima começar.

Não buscamos construir tudo de uma vez.

Buscamos construir uma empresa sólida.

---

# Princípios

- Lançar antes de aperfeiçoar.
- Validar antes de expandir.
- Automatizar antes de escalar.
- Crescer de forma sustentável.
- Priorizar impacto sobre quantidade.

---

# Phase 1 — Foundation

Objetivo:

Construir uma base sólida para a empresa.

## Prioridades

### Marca

- identidade consolidada;
- documentação estratégica;
- posicionamento definido.

### Site

- site institucional completo;
- páginas principais;
- SEO técnico;
- acessibilidade;
- performance;
- analytics.

### Infraestrutura

- domínio;
- hospedagem;
- e-mail profissional;
- versionamento;
- CI/CD.

### Conteúdo

- primeiros artigos;
- primeiros projetos publicados;
- páginas de serviços.

### Comercial

- formulário de contato;
- WhatsApp;
- processo de atendimento;
- proposta comercial.

---

# Critério de conclusão

A Fyrmma possui presença digital profissional e está pronta para receber clientes.

---

# Phase 2 — Launch

Objetivo:

Começar a adquirir clientes.

## Prioridades

### Marketing

- Google Business Profile;
- LinkedIn;
- Instagram;
- SEO contínuo.

### Prospecção

- empresas locais;
- Google Maps;
- networking;
- indicações.

### Conteúdo

- artigos técnicos;
- estudos de caso;
- materiais educativos.

### Processos

- CRM;
- pipeline comercial;
- acompanhamento de propostas.

---

# Critério de conclusão

Primeiros clientes recorrentes.

Processo comercial validado.

---

# Phase 3 — Growth

Objetivo:

Escalar operação sem perder qualidade.

## Prioridades

### Automação

- automações internas;
- templates;
- documentação;
- IA aplicada.

### Portfólio

- novos cases;
- novos segmentos;
- depoimentos.

### Conteúdo

- calendário editorial;
- vídeos;
- redes sociais;
- newsletter.

### Processos

- padronização;
- métricas;
- melhoria contínua.

---

# Critério de conclusão

Empresa operando com processos consistentes e geração previsível de oportunidades.

---

# Phase 4 — Platform

Objetivo:

Transformar conhecimento interno em produtos.

## Prioridades

### Produtos

- plataforma própria;
- ferramentas SaaS;
- soluções reutilizáveis.

### Inteligência Artificial

- agentes especializados;
- automações avançadas;
- ferramentas internas.

### Escalabilidade

- arquitetura multi-tenant;
- APIs;
- integrações.

---

# Critério de conclusão

Receita recorrente proveniente de produtos além de serviços.

---

# Processo de Revisão

O roadmap deve ser revisado periodicamente.

Mudanças devem ocorrer apenas quando houver aprendizado suficiente para justificar uma nova direção estratégica.

Evitar alterar prioridades por tendências momentâneas.

---

# Fora do Escopo

Este documento não contém:

- backlog;
- tarefas técnicas;
- bugs;
- issues;
- cronogramas detalhados;
- estimativas.

Esses itens pertencem às ferramentas de gerenciamento de projetos.

---

# Relação com os Demais Documentos

- BUSINESS.md define quem somos.
- PRINCIPLES.md define como tomamos decisões.
- BRAND.md define como queremos ser percebidos.
- WEBSITE.md define a arquitetura do site.
- COPY.md define nossa comunicação.
- ROADMAP.md define como evoluímos.
- AGENTS.md define como os agentes de IA devem trabalhar.
